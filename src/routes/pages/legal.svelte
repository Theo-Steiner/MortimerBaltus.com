		...
		<section on:click={() => (copyright = !copyright)} class:open={copyright}>
			...
			{#if copyright}
				<p transition:slide>
					...
					work are allowed only for private use, so must not serve either directly or indirectly for
					earnings. Unauthorized utilization of copyrighted works is punishable (§ 106 of the
					copyright law). <br /> <br />© {new Date().getFullYear()} MortimerBaltus. All rights reserved.
				</p>
			{/if}
		</section>
		<hr />
